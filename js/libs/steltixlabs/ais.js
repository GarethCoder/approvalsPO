define(['jquery'], function ($) {
  ...
  class AisClient {
    ...
    async fetch(query, mapper = simpleFieldMapper) {
      ...
      if (query.hasOwnProperty('targetName')) {
        const results = http.post(`${this.url}/dataservice`, {
    ...
    simpleAppStackCall(appStackJson) {
      let df = $.Deferred();
      
      $.ajax({
        url: `${this.url}/appstack`,
        data:JSON.stringify(appStackJson),
        contentType: 'application/json',
        method: 'post',
        type:'json'
    ...
    async _ensureToken(forceNew) {
      ...
      //console.log(JSON.stringify(this));
      const response = await http.post(`${this.url}/tokenrequest`, {
    ...
    async logOut(token){
      const response = await http.post(`${this.url}/v2/tokenrequest/logout`, {
        token: token,
      }, this.verbose);
    }
    async getUserInfo() {
  ...
  buildFormrequestJson = function ({ inputs = [], actions = [], formName, token, returnControlIDs = undefined}) {
    const reqObj = {
      returnControlIDs,
      token,
      'version': 'ZJDE0001',
